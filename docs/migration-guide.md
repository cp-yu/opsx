...
Existing `## 1. Actions` / `## 2. Checks` files still work. New changes should use coarse Task sections so `/opsx:apply` can run behavior checks through strict red/green TDD and mark checks complete after evidence passes.
