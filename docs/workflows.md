...
> [!NOTE]
> OpenSpec's managed workflow surface is **skills-only**. The `/opsx:*` references below describe the user-facing workflow entry points; OpenSpec installs them as skill files under `.<tool>/skills/`. Slash command files generated by older installations may still exist on disk — OpenSpec no longer generates, refreshes, or removes them.
