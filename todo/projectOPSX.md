...
| **Domain** | id, name, intent | 稳定问题域的边界和词汇表 |
| **Capability** | id, name, intent, status, spec_refs | 一个"值得路由"的能力单元 |
...
| 关系 | 含义 |
|------|------|
| `contains` | 包含（Project → Domain → Capability） |
| `depends_on` | 依赖（Capability A 依赖 Capability B） |
| `constrains` | 约束（Invariant 约束 Capability/Interface） |
| `implemented_by` | 实现（Capability 由 Evidence 实现） |
| `verified_by` | 验证（Capability 被 Evidence 验证） |
| `modified_by` | 修改（节点被 Change 修改，从 opsx-delta 机械生成） |
...
```
OPSX SUBSUMES Specs:
- Project OPSX 是 source of truth（语义图）
- openspec/specs/ 是 OPSX 的展开视图（行为规范的详细正文）
...
OPSX delta 是**执行决策时的修改记录**，不是待执行的修改指令。
...
| 编号 | 事项 | 状态 |
|------|------|------|
| V0 | 为 OpenSpec 自身写一个真实的 project.opsx.yaml，验证 AI 认知提升 | 完成 |
