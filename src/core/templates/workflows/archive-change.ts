...
function buildAgentGitFlowStep(): string {
  return `   The agent continues the post-archive git flow. First handle the implementation boundary before OpenSpec/docs archive artifacts. If uncommitted real project implementation changes remain, create a normal implementation commit that contains only those changes. Then always create a semantic boundary commit with \`git commit --allow-empty\`; this boundary commit may be intentionally empty when the effective implementation diff is already carried by retained \`wip: opt-*\` checkpoint commits. If \`git.commitMessage.boundary\` is set, read that project-relative path; otherwise read \`${BOUNDARY_COMMIT_MESSAGE_SHARED_PATH}\`. Use that template to build the boundary commit message and run \`git commit -F -\` for the boundary commit. If \`git.commitMessage.archive\` is set, read that project-relative path; otherwise read \`${ARCHIVE_COMMIT_MESSAGE_SHARED_PATH}\`. Use that template before creating the OpenSpec/docs archive commit, add only archive/synced paths, and run \`git commit -F -\`. If a merge or squash commit message is needed, prepare it from the configured or built-in merge template. If \`git.commitMessage.merge\` is set, read that project-relative path; otherwise read \`${MERGE_SUMMARY_MESSAGE_SHARED_PATH}\`. Apply \`git.merge.strategy\` with \`git merge --no-ff\`, \`git merge --ff-only\`, or \`git merge --squash\` as appropriate. Use \`git.branch.deleteAfterArchive\` only after confirming the branch is merged with \`git branch --merged\`. Build paths with \`path.join()\`, \`path.resolve()\`, and \`path.normalize()\`.`;
}
