...
Use the Agent tool to spawn a subagent. Pass the subagent a prompt instructing it to load the \`openspec-impact-sweeper\` skill with \`projectRoot\`, \`concept\`, optional \`optionalChangeName\`, optional \`knownUserTerms\`, and optional \`focus\`. Treat each new concept as an independent sweep, even if another concept was already swept earlier in the conversation. After the subagent returns the JSON report path, read the JSON file content (not just the path string), interpret the findings, and continue the conversation by presenting terminology observations or impact questions to the user.
