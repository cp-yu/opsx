...
openspec-verify-change skill 的 SkillTemplate MUST 包含 `referenceFiles` 字段，其中包含完整的 Phase 2 checkpoint 协议文档。主 instructions MUST 明确引用该 reference 文档。
...
- **AND** 数组 MUST 包含一个 reference，其 `path` 为 `'references/phase2-checkpoint-protocol.md'`
...
- **AND** 该章节 MUST 列出 `references/phase2-checkpoint-protocol.md`
- **AND** Phase 2 相关步骤 MUST 提示 "follow the checkpoint protocol in references/phase2-checkpoint-protocol.md"
...
#### Scenario: 生成的 skill 文件包含 reference

- **WHEN** 通过 workflow-installation.ts 生成 skill 文件
- **THEN** `.claude/skills/openspec-verify-change/` 目录 MUST 包含：
  - `SKILL.md`（主文件）
  - `references/phase2-checkpoint-protocol.md`（reference 文件）
- **AND** `SKILL.md` MUST 引用 `references/phase2-checkpoint-protocol.md`
- **AND** reference 文件内容 MUST 与 `PHASE2_CHECKPOINT_PROTOCOL_REFERENCE` 常量一致
