...
`opsx-archive-skill` SHALL 在现有归档流程的 mv 与 sync 完成之后，按顺序追加 archive commit、merge to originalBranch、可选 branch 清理三个步骤。
...
- **THEN** 摘要 SHALL 包含以下字段：
  - 是否创建了 archive commit（commit SHA 或 "skipped: no changes"）
  - merge 策略（no-ff / ff-only / squash / skipped: no isolation）
  - merge 结果（success: <merge SHA> / abort: conflict / skipped）
  - feature 分支处理（deleted / kept: branch not merged / kept: config disabled）
...
- **WHEN** archive 需要决定 merge 策略与 messageFrom
- **THEN** archive SHALL 从 prompt/runtime projection 读取 `git.merge.strategy`、`git.merge.messageFrom`、`git.branch.deleteAfterArchive`
- **AND** SHALL NOT 在模板正文里直接 `yaml.parse(config.yaml)`
- **AND** projection 缺失字段时 SHALL 使用默认值（`no-ff` / `artifacts` / `false`）
