...
Explore 在已有 change 上提供制品更新建议时 SHALL 先判断 insight 类型，只有可观察行为需求或可观察行为变化进入 `specs/<capability>/spec.md`。

#### Scenario: 可观察行为进入 specs

- **WHEN** explore 发现新的可观察行为需求或现有可观察行为变化
- **THEN** 系统 SHALL 建议将其捕获到 `specs/<capability>/spec.md`
- **AND** 建议文案 SHALL 表明这是 observable behavior，而不是泛化的 "new requirement"

#### Scenario: 重构和实现决策进入 design

- **WHEN** explore 形成 refactor rationale、rejected path、implementation strategy 或不再使用旧路径的决策
- **THEN** 系统 SHALL 建议将其捕获到 `design.md`
- **AND** SHALL NOT 建议将这些非行为内容写入 `specs/<capability>/spec.md`
...
- **THEN** scope change SHALL 路由到 `proposal.md`
- **AND** new work 或 verification work SHALL 路由到 `tasks.md`
- **AND** OPSX graph intent change SHALL 路由到 `opsx-delta.yaml`
- **AND** invalidated assumption SHALL 路由到相关制品
